...
def read_data(file_path, msg_types):    
    try:
        ...
        while True:
            ...
            data_collector[msg_type].append(msg.to_dict())
        
        page_contents = []
        for msg_type in data_collector: 
            content = json.dumps(data_collector[msg_type])
...
#     def store_file_content(self, session_id, filename, content):
#         """Store file content in Redis"""
#         file_id = hashlib.md5(f"{session_id}_{filename}".encode()).hexdigest()[:8]
#         key = f"session:{session_id}:file:{file_id}"
...
#     content = f.read()
#     file_id = cache.store_file_content("user123", "document.txt", content)

# # Retrieve file
# file_data = cache.get_file_content("user123", file_id)
# print(file_data["content"])

# # Delete file
# cache.delete_file("user123", file_id)